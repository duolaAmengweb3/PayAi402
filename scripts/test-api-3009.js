...
async function runTests() {
  ...
  console.log(`\n✅ 服务已部署到: http://localhost:3009`);
  console.log(`✅ 测试页面: http://localhost:3009/test\n`);
...
test('主页应可访问', async () => {
  ...
  if (res.status !== 200) {
    throw new Error(`期望状态码 200, 得到 ${res.status}`);
  }

  if (!res.data.includes('PayAI402')) {
    throw new Error('主页内容不正确');
  }
});

// Test 6: Test Page Accessible
test('测试页面应可访问', async () => {
